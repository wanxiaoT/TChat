...
- **語音朗讀（TTS）功能**
  ...
  - 支援選擇不同的 TTS 引擎
  - TTS 引擎選項採用卡片式設計
  - 使用 `synthesizeToFile()` 實現更穩定的語音合成

- **Cloudflare R2 雲端備份**
  - 支援將資料備份到 Cloudflare R2 雲端儲存
  - 大檔案採用串流上傳，優化記憶體佔用
  - 完整的備份/還原功能
...
**實現**：
- `TtsService` 語音合成服務
- 使用 `synthesizeToFile()` 替代 `speak()` 提高穩定性
- 支援多種 TTS 引擎選擇
- 卡片式引擎選擇 UI

---

#### 2. Cloudflare R2 雲端備份
...
| 模組 | 檔案 | 說明 |
|------|------|------|
| app | TtsService.kt | TTS 語音合成服務 |
| app | TtsSettingsScreen.kt | TTS 設定頁面 |
| app | R2BackupService.kt | R2 雲端備份服務 |
| app | R2BackupScreen.kt | 雲端備份設定頁面 |
| app | ApiKeyRotationManager.kt | API Key 輪詢管理 |
| app | ChatDrawer.kt | 側邊欄搜尋功能 |
| app | SettingsScreen.kt | 設定頁面圖示更新 |
